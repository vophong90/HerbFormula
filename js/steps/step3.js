...
async function autoSuggestClassicSyndrome() {
  ...
  if (!outputBox) {
    console.warn("#syndrome-classic không tồn tại trong DOM.");
    return;
  }
...
3. Trình bày kết quả như sau (chỉ văn bản, không mã hóa, không JSON, nếu người dùng nhập tiếng Trung thì trả kết quả là tiếng Trung chuyên ngành Trung y):
...
`.trim();
  ...
  try {
    const res = await fetch("https://gpt-api-19xu.onrender.com/gpt.php", {
      ...
      body: JSON.stringify({
        action: "chat",   // QUAN TRỌNG: khớp router PHP (dùng /v1/responses)
        prompt
    ...
    outputBox.value = gptText;

    // Lưu localStorage
    data.steps = data.steps || {};
    data.steps.step3 = { ...data.steps.step3, classic: gptText };
    localStorage.setItem("currentData", JSON.stringify(data));
...
// Helper: trích văn bản từ Responses API (và tương thích ngược Chat Completions)
function extractOutputText(resp) {
  // 1) Responses API: thuộc tính tiện dụng
...
async function autoGenerateDiagnosticTree() {
  ...
  // 1) Sắp xếp triệu chứng theo VAS giảm dần
  const sorted = [...symptoms].sort((a, b) => Number(b.vas) - Number(a.vas));
  const symptomList = sorted.map(s => `– ${s.symptom} (VAS: ${s.vas})`).join("\n");

  // 2) Lấy thông tin bổ sung nếu có
  const extraAnswers = (typeof collectExtraAnswers === "function") ? collectExtraAnswers() : [];
  let extraText = "";
  if (Array.isArray(extraAnswers) && extraAnswers.length > 0) {
...
Tôi sẽ cung cấp danh sách các triệu chứng và điểm VAS (mức độ nặng nhẹ). Bắt buộc phản hồi về phải là tiếng Trung nếu dữ liệu gửi lên có tiếng Trung.  
...
– **Đàm thấp**: triệu chứng nặng nề, bài tiết ra các chất tiết đục, dơ, mùi tanh hôi  
...
– **Tinh bất túc**: người gầy, lưng gối mỏi, giảm trí nhớ, biểu hiện suy giảm công năng của tủy, não  
...
${extraText}`;
  ...
  // 4) Gọi GPT qua gpt.php (Responses API)
  try {
    const res = await fetch("https://gpt-api-19xu.onrender.com/gpt.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: "chat",        // QUAN TRỌNG: đúng router PHP
        prompt: basePrompt
        // model: "gpt-5"      // (tuỳ chọn) override nếu cần
      })
    });
